...
especialidades = list(df.especialidad.unique())
planes = list(df.plan.unique())
...
prestadores = prestadores.unstack(level='plan', fill_value=set())
prestadores.columns = prestadores.columns.droplevel()
prestadores = prestadores.reset_index()
properties = [column for column in prestadores.columns if column not in planes + ['coordenadas']]

geojson = prestadores[
  prestadores['coordenadas'].str.contains(r'^-?\d+.\d+,-?\d+.\d+$', regex=True)
].reset_index(drop=True)
geojson['especialidades'] = geojson[planes].apply(lambda x: x.to_dict(), axis=1)
...
geojson = geojson.drop(planes + properties + ['coordenadas'], axis=1)
...
with open('especialidades.json', 'w') as fileout:
  json.dump(especialidades, fileout)
with open('planes.json', 'w') as fileout:
  json.dump(planes, fileout)